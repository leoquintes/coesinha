// package.json - Este ficheiro diz ao Render quais ferramentas instalar.
// Copie este conteúdo e guarde-o num ficheiro chamado package.json

{
  "name": "bot-whatsapp-ai",
  "version": "1.0.0",
  "description": "Bot de horários para WhatsApp com IA do Gemini",
  "main": "bot.js",
  "scripts": {
    "start": "node bot.js"
  },
...
```javascript
// bot.js - Este é o cérebro do seu bot.
// Copie este conteúdo e guarde-o num ficheiro chamado bot.js
...
// --- LÓGICA DE HORÁRIOS (A mesma de antes) ---
const allStops = [
...
// ... (o resto das funções de cálculo de horário permanecem as mesmas)
...
function calculateBusArrivalsForStop(stop, destination) { const line = destination === 'Rio de Janeiro' ? companyBusData.lines.ida : companyBusData.lines.volta; if (!line || !line.path.includes(stop.id)) return []; const timeZone = 'America/Sao_Paulo'; const now = new Date(); const dayOfWeek = parseInt(format(now, 'i', { timeZone })); if (dayOfWeek === 7 || dayOfWeek === 6) { return []; } const upcomingArrivals = []; const MAX_ARRIVALS_TO_SHOW = 3; for (const trip of line.trips) { const travelTimes = generateProportionalTravelTimes(line.path, trip.duration); const travelTime = travelTimes[stop.id]; if (typeof travelTime === 'undefined') continue; const [hours, minutes] = trip.departure.split(':').map(Number); const departureString = `${format(now, 'yyyy-MM-dd', { timeZone })}T${String(hours).padStart(2, '0')}:${String(minutes).padStart(2, '0')}:00`; const departureUTC = zonedTimeToUtc(departureString, timeZone); const arrivalUTC = new Date(departureUTC.getTime() + travelTime * 60000); if (arrivalUTC > now) { const minutesAway = Math.round((arrivalUTC.getTime() - now.getTime()) / 60000); upcomingArrivals.push({ line: line.line, destination: line.destination, minutesAway: minutesAway, arrivalTime: arrivalUTC }); if (upcomingArrivals.length >= MAX_ARRIVALS_TO_SHOW) break; } } return upcomingArrivals; }

// --- LÓGICA PRINCIPAL DO BOT COM IA ---

async function handleIncomingMessageWithAI(incomingMsg) {
    // 1. Primeiro, tentamos a correspondência exata/parcial que já tínhamos, pois é mais rápida e barata.
    ...
    if (foundStop) {
        // Se encontrarmos uma correspondência direta, calculamos e formatamos a resposta como antes.
    ...
    // 2. Se não houver correspondência direta, usamos a IA do Gemini.
    try {
        ...
        if (aiResponse.includes('<found_stop>')) {
            ...
            if (stopData) {
                // Se a IA encontrou um ponto, calculamos os horários e pedimos para a IA formatar a resposta.
        ...
        // Se a IA não retornou a tag, significa que é uma resposta conversacional.
        return aiResponse;
