...
function generateTravelTimes(path, trip) {
    const travelTimes = {};
    const findStop = (id) => allStops.find(s => s.id === id);

    // Ponto de divisão da rota e velocidade do primeiro segmento
    const splitPointId = "137680658"; // Barreto ida
    const speedSegment1 = 50; // km/h

    const splitIndex = path.indexOf(splitPointId);

    // Se a rota for a de "Ida" e o ponto de divisão existir
    if (trip.destination === 'Rio de Janeiro' && splitIndex !== -1) {
        // --- CÁLCULO PARA A ROTA DE IDA (COM 2 VELOCIDADES) ---

        // 1. Calcular distância e tempo para o Segmento 1 (até Barreto)
        let distanceSegment1 = 0;
        for (let i = 0; i < splitIndex; i++) {
            const stopA = findStop(path[i]);
            const stopB = findStop(path[i + 1]);
            distanceSegment1 += calculateDistance(stopA.lat, stopA.lon, stopB.lat, stopB.lon);
        }
        const timeToSplitPoint = (distanceSegment1 / speedSegment1) * 60; // em minutos

        // 2. Calcular tempo e distância para o Segmento 2 (depois de Barreto)
        const timeSegment2 = trip.duration - timeToSplitPoint;
        let distanceSegment2 = 0;
        for (let i = splitIndex; i < path.length - 1; i++) {
            const stopA = findStop(path[i]);
            const stopB = findStop(path[i + 1]);
            distanceSegment2 += calculateDistance(stopA.lat, stopA.lon, stopB.lat, stopB.lon);
        }

        // 3. Gerar tempos de viagem para cada paragem
        let cumulativeDistance = 0;
        travelTimes[path[0]] = 0;
        for (let i = 0; i < path.length - 1; i++) {
            const stopA = findStop(path[i]);
            const stopB = findStop(path[i + 1]);
            cumulativeDistance += calculateDistance(stopA.lat, stopA.lon, stopB.lat, stopB.lon);

            if (i < splitIndex) {
                // Paragens no Segmento 1: cálculo proporcional com base na velocidade de 50km/h
                travelTimes[path[i + 1]] = Math.round((cumulativeDistance / distanceSegment1) * timeToSplitPoint);
            } else {
                // Paragens no Segmento 2: cálculo proporcional com base no tempo e distância restantes
                const distanceFromSplit = cumulativeDistance - distanceSegment1;
                const timeAfterSplit = (distanceFromSplit / distanceSegment2) * timeSegment2;
                travelTimes[path[i + 1]] = Math.round(timeToSplitPoint + timeAfterSplit);
            }
        }
    } else {
        // --- CÁLCULO PADRÃO (PARA A ROTA DE VOLTA OU SE O PONTO DE DIVISÃO NÃO EXISTIR) ---
        return generateProportionalTravelTimes(path, trip.duration);
    }
